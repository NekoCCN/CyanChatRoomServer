...
public class GetFriendshipListCommandHandler implements CommandHandler
{
    ...
    {
        CommandHelper.withAuthenticatedUser(ctx, json_request, MessageType.GET_FRIENDSHIP_LIST_REQUEST, (UUID operator_id) ->
        {
            ...
            try
            {
                List<Friendship> friends = friendship_application_service_.getFriendshipList(payload.user_id());
                ProtocolMessage<FriendshipListResponse> response_msg =
                        new ProtocolMessage<>(MessageType.GET_FRIENDSHIP_LIST_RESPONSE,
